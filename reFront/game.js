...
async function uploadFiles(){
  ...
  if(candidates.length+files.length>10){
  ...
  busy=false;
  uploadButton.disabled=false;
  if(candidates.length===0)setStatus("対戦に使用できるレシートがありません。別のレシート画像をアップロードしてください。",true);
}
